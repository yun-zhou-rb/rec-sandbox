...
def citeulike(tag_occurence_thres=10):
    ...
    for u, item_list in enumerate(open("data/citeulike-t/users.dat").readlines()):
    ...
    n_features = 0
    for l in open("data/citeulike-t/tag-item.dat").readlines():
        items = l.strip().split(" ")
        if len(items) >= tag_occurence_thres:
            n_features += 1
    print("{} features over tag_occurence_thres ({})".format(n_features, tag_occurence_thres))
    item_features = dok_matrix((n_items, n_features), dtype=np.int32)
    feature_index = 0
    for l in open("data/citeulike-t/tag-item.dat").readlines():
        items = l.strip().split(" ")
        if len(items) >= tag_occurence_thres:
            item_features[[int(i) for i in items], feature_index] = 1
            feature_index += 1
